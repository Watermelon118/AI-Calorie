...
| 目标设置 | 展示体重趋势、目标进度、记录体重入口和设置目标入口；两个入口分别使用底部滑出页面，保存逻辑和接口后续补充 | 目标页入口交互进行中 |
| 历史记录 | 查看过往日期的餐食记录与统计，支持进入单日明细，右上角 AI 顾问和单日补录均使用底部滑出静态页 | 静态页完成 |
...
| 历史 | `/history` | 日期列表、单日明细、历史统计入口、AI 顾问底部滑出页、补录食物底部滑出页 | 静态页完成 |
| 统计 | `/stats` | 日维度热量统计和趋势展示 | 未开始 |
| 目标 | `/aims` | 体重趋势、目标进度、记录体重滑出页和设置目标滑出页 | 目标页入口交互进行中 |
...
- 底部 TabBar 当前承载：今日、记录、目标、历史四个主入口。
